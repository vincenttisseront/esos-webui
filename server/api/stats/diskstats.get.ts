import { readDiskStatSnapshots } from '../../utils/io-stats-reader'
import { pushDiskStatSnapshots } from '../../utils/metrics-store'
...
export default defineEventHandler(async (event) => {
  ...
  const run = async () => {
    ...
    try {
      const snapshots = await readDiskStatSnapshots()
      const throughputs = pushDiskStatSnapshots(snapshots, bucketId)
      return { disks: throughputs, capturedAt: Date.now() }
    } catch {
      setResponseStatus(event, 503)
      return { error: 'SSH non disponible' }
    }
