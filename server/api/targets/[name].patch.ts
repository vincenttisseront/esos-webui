import { setTargetEnabled } from '~/server/utils/scst-config-writer'
...
export default defineEventHandler(async (event) => {
  ...
  if (typeof body?.enabled !== 'boolean') {
    throw createError({ statusCode: 400, statusMessage: 'enabled (boolean) requis' })
  }

  try {
    await setTargetEnabled(name, body.enabled)
  } catch (err: unknown) {
