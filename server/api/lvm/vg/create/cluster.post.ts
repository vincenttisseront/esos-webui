...
import type { ClusterLvmDiskMapping, VgCreatePayload } from '../../../../utils/lvm-types'

export default defineEventHandler(async (event) => {
  const sanId = requireSanIdQuery(event)
  const body = await readBody<VgCreatePayload & {
  ...
  const plan = await buildClusterVgCreatePlan(
