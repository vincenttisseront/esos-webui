<template>
  <LvmWizardModalShell :title="t('lvm.remove.vg.title')" :step="step" :total-steps="2" icon="i-heroicons-trash">
    ...
    <div v-else-if="step === 1" class="space-y-4">
      ...
      <p class="text-sm">{{ t('lvm.remove.vg.intro', { name: vgName }) }}</p>
    ...
    <template #footer>
      <div class="flex justify-end gap-2">
        ...
        <UButton v-if="step === 1" color="red" :loading="busy" :disabled="!canExecute" @click="execute">{{ t('lvm.remove.vg.execute') }}</UButton>
      </div>
    </template>
  </LvmWizardModalShell>
</template>
...
const props = defineProps<{ sanId: string; vgName: string; isClustered?: boolean; clusterId?: string }>()
...
const plan = ref<Awaited<ReturnType<typeof lvm.planClusterVgRemove>> | null>(null)
const executionResult = ref<Awaited<ReturnType<typeof lvm.executeClusterVgRemove>> | null>(null)
...
async function loadPreflight() {
  ...
  try {
    if (props.isClustered && props.clusterId) {
      plan.value = await lvm.planClusterVgRemove({ name: props.vgName, confirmation: '', clusterExecution: { primarySanId: props.sanId, clusterId: props.clusterId } })
      blockers.value = plan.value.blockers
    } else {
      const pre = await lvm.preflight({ action: 'vgremove', payload: { name: props.vgName, confirmation: '' } })
      blockers.value = pre.blockers
      requiredConfirmation.value = pre.requiredConfirmation
      standalonePreview.value = pre.commandPreview ?? `vgremove -y -f ${props.vgName}`
    }
  } catch (e: unknown) {
...
async function execute() {
  ...
  try {
    if (props.isClustered && props.clusterId && plan.value) {
      executionResult.value = await lvm.executeClusterVgRemove({ name: props.vgName, confirmation: confirmation.value.trim(), clusterExecution: { primarySanId: props.sanId, clusterId: props.clusterId } })
      step.value = 2
      if (executionResult.value.success) { toast.success(t('lvm.vg.removed')); emit('close') }
      else toast.error(t('lvm.remove.partial_failure'), executionResult.value.errors.join(' · '))
    } else {
      await lvm.removeVg({ name: props.vgName, confirmation: confirmation.value.trim() })
      toast.success(t('lvm.vg.removed'))
      emit('close')
    }
  } catch (e: unknown) {
