<template>
  <div class="rounded-lg border border-gray-200 bg-white p-4 space-y-3">
    ...
    <div v-if="!installed.raw || installed.buildType === 'unknown'" class="rounded-lg bg-gray-50 border border-gray-200 p-3 text-sm text-gray-500 flex items-center gap-2">
    ...
    <template v-else-if="installed.buildType === 'stable'">
      <div class="flex items-center gap-3">
        <div class="w-10 h-10 rounded-full bg-blue-50 border border-blue-200 flex items-center justify-center shrink-0">
          <UIcon name="i-heroicons-tag" class="w-5 h-5 text-blue-500" />
        </div>
        <div>
          <p class="text-xl font-bold font-mono text-gray-900">{{ installed.version }}</p>
          <p class="text-xs text-gray-400">Build stable</p>
        </div>
      </div>
      <div class="text-xs text-gray-400 font-mono bg-gray-50 rounded px-3 py-2 border border-gray-100">
        {{ installed.raw }}
      </div>
    </template>
    ...
    <template v-else-if="installed.buildType === 'master'">
      <div class="flex items-center gap-3">
        ...
        <div>
          <p class="text-sm font-bold font-mono text-gray-900">{{ installed.branch }}</p>
